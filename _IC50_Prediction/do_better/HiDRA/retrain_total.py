...
parser.add_argument('-choice', type=int, default=0, help='Test type [Normal & Blind]')
parser.add_argument('-dir_in', type=str, default="_data/ProcessedFile", help='Input directory')

parser.add_argument('-nth', type=int, default=0, help='KFold Index')
...
def main(args):
    ...
    batch_size = 1024
    train_loader = DataLoader(ic50_data, data, args, batch_size=batch_size, shuffle=True)
    # train_loader = DataLoader(ic50_train, data, args, batch_size=batch_size, shuffle=True)
    # valid_loader = DataLoader(ic50_valid, data, args, batch_size=batch_size, shuffle=False)
    # test_loader = DataLoader(ic50_test, data, args, batch_size=batch_size, shuffle=False)
    ...
    # https://stackoverflow.com/questions/61258124

    print("Steps [Train] : {}".format(steps_train))
    # print("Steps [Test] : {}".format(steps_test))
    # print("Steps [Valid] : {}".format(steps_valid))
    
    # The number of parameters : 2,031,724
    # verbose = True
    # if verbose : print(model.summary())
    
    print("\n### Train Start!")
    check = ModelCheckpoint(args.dir_param, monitor='train_loss', mode='min', verbose=1, save_best_only=False, save_weights_only=True)
    # stop = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=patience, restore_best_weights=True)
    # check = ModelCheckpoint(args.dir_param, monitor='val_loss', mode='min', verbose=1, save_best_only=True, save_weights_only=True)
    # history = model.fit(train_loader, validation_data=valid_loader, epochs=epoch, verbose=1, callbacks=[stop, check])
    
    from time import time, ctime
    start = time()
    print("Time Start : {}".format(ctime(start)))
    
    history = model.fit_generator(train_loader, validation_data=None, steps_per_epoch=steps_train,  
                                  workers=args.cpu, epochs=epoch, verbose=1, callbacks=[check])
    
    # history = model.fit_generator(train_loader, validation_data=test_loader, 
    #                               steps_per_epoch=steps_train, validation_steps=steps_test, 
    #                               workers=args.cpu, epochs=epoch, verbose=1, callbacks=[stop, check])
    
    end = time()
    print("Time End : {}".format(ctime(end)))
    print('time elapsed:', end - start)
    
    # # Valid Performance
    # print("\n### Valid Performance...")
    # # pred_valid = model.predict(valid_loader)
    # pred_valid = model.predict_generator(valid_loader, workers=args.cpu)
    # pred_to_csv(pred_valid, ic50_valid, args.dir_valid)
    
    # Test Performance
    # print("\n### Test Performance...")
    # pred_test = model.predict(test_loader)
    # pred_test = model.predict_generator(train_loader, workers=args.cpu)
    # pred_to_csv(pred_test, ic50_test, args.dir_test)
    
    pred_train = model.predict_generator(train_loader, workers=args.cpu)
    pred_to_csv(pred_train, ic50_data, args.dir_pred)
    
    # # Model Parameter
    # model.save_weights(args.dir_param)
