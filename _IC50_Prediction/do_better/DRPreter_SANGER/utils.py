...
def load_data(ic50_train, ic50_valid, ic50_test, drug_dict, cell_dict, edge_index, args): # For PPI network
    
    collate_fn = _collate
    train_dataset = MyDataset(drug_dict, cell_dict, ic50_train, edge_index, args)
    val_dataset = MyDataset(drug_dict, cell_dict, ic50_valid, edge_index, args)
    test_dataset = MyDataset(drug_dict, cell_dict, ic50_test, edge_index, args)
    
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn, num_workers=args.cpu)
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn, num_workers=args.cpu)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn, num_workers=args.cpu)

    return train_loader, val_loader, test_loader


def load_data_retrain(ic50_train, ic50_test, drug_dict, cell_dict, edge_index, args) :
...
def load_data_test(ic50_test, drug_dict, cell_dict, edge_index, args, shuffle=False): # For PPI network
    ...
    collate_fn = _collate
    test_dataset = MyDataset(drug_dict, cell_dict, ic50_test, edge_index, args)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=shuffle, collate_fn=collate_fn, num_workers=args.cpu)
...
class Dataset_Sim(Dataset):
    def __init__(self, drug_name2idx, cell_name2idx, ic50_data, args):
        super(Dataset_Sim, self).__init__()
        self.drug_name2idx = drug_name2idx
        self.cell_name2idx = cell_name2idx
        
        col_drug = args.col_drug
        col_cell = args.col_cell
        col_ic50 = args.col_ic50
        
        self.drug_name = ic50_data[col_drug]
        self.Cell_line_name = ic50_data[col_cell]
        self.value = ic50_data[col_ic50]

    def __len__(self):
        return len(self.value)

    def __getitem__(self, index):
        return (self.drug_name2idx[self.drug_name[index]], self.cell_name2idx[self.Cell_line_name[index]], self.value[index])


def load_sim_data(ic50_train, ic50_valid, ic50_test, drug_name2idx, cell_name2idx, args):
    
    train_data = Dataset_Sim(drug_name2idx, cell_name2idx, ic50_train, args)
    val_data = Dataset_Sim(drug_name2idx, cell_name2idx, ic50_valid, args)
    test_data = Dataset_Sim(drug_name2idx, cell_name2idx, ic50_test, args)
    
    num_workers = 9 if args.device=="cuda" else 0
...
def load_sim_data_test(ic50_test, drug_name2idx, cell_name2idx, args):
...
def load_sim_graph(drug_data, cell_data, edge_index, weight_dir, knn_dir, args):  
    ...
    drug_graph = list(drug_data.values())
    cell_graph = list(cell_data.values())
    ...
    model.load_state_dict(torch.load(weight_dir, map_location=args.device)['model_state_dict'])
