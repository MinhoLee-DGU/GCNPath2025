...
def create_dir_out_(args, dir_out=None, suf_param="pth", retrain=False) :
    ...
    if args.sim==1 :
        args.dir_drpreter = "./Results/{}/{}/DRPreter/param_{}.pth".format(dir1, dir2, args.nth)
...
def summary_model(model, verbose=True) :
    ...
    param_cell = sum(p.numel() for p in model.CellEncoder.parameters())
    param_drug = sum(p.numel() for p in model.DrugEncoder.parameters())
    param_attn = sum(p.numel() for p in model.Transformer.parameters())
    ...
    print("# Model Parameters : {} [Attn]".format(param_attn))
