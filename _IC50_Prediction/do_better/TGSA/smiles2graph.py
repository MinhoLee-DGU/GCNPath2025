...
# def save_drug_graph():
#     # smiles = pd.read_csv('./_data/SMILES_GDSC.csv')
#     
#     drug_dict = {}
#     for i in range(len(smiles)):
#         drug_dict[str(smiles.iloc[i, 0])] = smiles2graph(smiles.iloc[i, 2])
#         
#     np.save('./_data/drug_feature_graph.npy', drug_dict)
#     
#     drug_name2smiles_dict = {k:v for k,v in zip(smiles["Drug_CID"].astype(str), smiles["SMILES_CAN"])}
#     with open("./_data/drug_name2smiles_dict", "wb") as f :
#         pickle.dump(drug_name2smiles_dict, f)
...
def save_drug_graph(file_in, file_out, col_drug="Drug_CID", col_smi="SMILES_CAN"):
    ...
    for i in range(len(smiles)):
        ...
        # drug_dict[smiles.iloc[i, 0]] = smiles2graph(smiles.iloc[i, 2])
        no_bond = (drug_dict[smiles[col_drug][i]].edge_index.shape[1]==0)
        
        if no_bond : 
            del drug_dict[smiles[col_drug][i]]
            print("# Drug {} is not processed for unknown reason...".format(smiles[col_drug][i]))
            print("# Drug {} : {}".format(smiles[col_drug][i], smiles[col_smi][i]))
