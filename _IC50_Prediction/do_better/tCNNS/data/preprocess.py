...
def onehot_encode(char_list, smiles_string, length):
    ...
    ans = np.array(map(encode_row, char_list))   # [n_char, n_char_smi]
...
def smiles_to_onehot_external(smiles, c_chars, c_length):
    c_ndarray = np.ndarray(shape=(len(smiles), len(c_chars), c_length), dtype=np.float32)
    for i in range(len(smiles)):
        exceed_len = len(smiles[i])>c_length
        if exceed_len : smiles[i] = smiles[i][:c_length]
        c_ndarray[i, ...] = onehot_encode(c_chars, smiles[i], c_length)
...
def load_as_ndarray():
    ...
    reader = csv.reader(open(folder + "SMILES_GDSC.csv"))
    next(reader, None)
...
def save_drug_smiles_onehot():
    smiles = load_as_ndarray()
    # we will abandon isomerics smiles from now on
    c_chars, _ = charsets(smiles)   # 29
    c_length = max(map(len, map(string2smiles_list, list(smiles[:, 2]))))   # 188
    
    count = smiles.shape[0]
    drug_cids = smiles[:, 0].astype(str)
    # drug_cids = smiles[:, 0].astype(int)
    # drug_names = smiles[:, 0].astype(str)
    # drug_cids = smiles[:, 1].astype(int)
    smiles = [string2smiles_list(smiles[i, 2]) for i in range(count)]
    ...
    info_drug = True
    if info_drug :
        print "\n# Number of drugs in each alphabet"
        print save_dict["canonical"].sum(axis=1).sum(axis=0)   # (188, )
        print "\n# Number of alphabets in each drugs"
        print save_dict["canonical"].sum(axis=1).sum(axis=1)   # (432, )
        print "\n# Minimum number of alphabets"
        print save_dict["canonical"].sum(axis=1).sum(axis=1).min()   # 9
        print "\n# Maximum number of alphabets"
        print save_dict["canonical"].sum(axis=1).sum(axis=1).max()   # 188
        
def save_drug_smiles_onehot_chembl():
  
    import pandas as pd
    drug_ori = "drug_onehot_smiles.npy"
    drug_data = np.load(drug_ori, encoding="latin1", allow_pickle=True).item()
    
    drug_ext = "_data/SMILES_ChEMBL_tCNNS.csv"
    drug_data_ext = pd.read_csv(drug_ext)
    
    count = drug_data_ext.shape[0]
    drug_ids = drug_data_ext.Molecule_ChEMBL_ID.astype(str)
    smiles = [string2smiles_list(drug_data_ext.Canonical_SMILES[i]) for i in range(count)]
    
    # Process ChEMBL drugs with the information from GDSC drugs
    c_chars = drug_data["c_chars"]
    c_length = drug_data["canonical"].shape[2]
    
    exceed_len = np.array([len(smi)>c_length for smi in smiles]).sum()
    canonical = smiles_to_onehot_external(smiles, c_chars, c_length)
    print "# The number of drugs that exceed the maximum c_length [{}] : {}".format(c_length, exceed_len)
    
    # If saving all drugs, the error of file size occured... 
    # So decided to divide the drug data [148402]
    # OverflowError: cannot serialize a string larger than 2 GiB
    
    batch_size = 20000
    batch_num = int(math.ceil(len(drug_ids)/batch_size))
    
    for i in range(batch_num+1) :
        save_dict = {}
        idx1 = batch_size * i
        idx2 = batch_size * (i+1)
        idx2 = min(idx2, len(drug_ids))
        
        save_dict["drug_cids"] = drug_ids[idx1:idx2]
        save_dict["canonical"] = canonical[idx1:idx2]
        save_dict["c_chars"] = c_chars
        
        drug_ext = "drug_onehot_smiles_chembl{}.npy".format(i)
        print "Drug data with batch {} [n={}]".format(i, len(drug_ids[idx1:idx2]))
        np.save(drug_ext, save_dict)
...
# save_drug_smiles_onehot_chembl()

# We borrowed the preprocessed data from GraphDRP
