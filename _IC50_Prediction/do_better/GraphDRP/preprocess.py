...
def load_drug_smile(file, col_drug="Drug_CID", col_smi="SMILES_CAN"):
    drug_dict = {}
    reader = pd.read_csv(open(file))

    for i in range(reader.shape[0]) :
        name = reader[col_drug][i]
        smile = reader[col_smi][i]
        g = smile_to_graph(smile)
        drug_dict[str(name)] = g
        no_bond = (len(g[2])==0)
        
        if no_bond : 
            del drug_dict[str(name)]
            print("# Drug {} is not processed for unknown reason...".format(name))
            print("# Drug {} : {}".format(name, smile))
...
def save_cell_mut_matrix():
    ...
    return cell_dict, mut_dict, cell_feature
...
if __name__ == "__main__" :
    cell_dict, mut_dict, cell_feature = save_cell_mut_matrix()
    cell_feat = pd.DataFrame(cell_feature, index=cell_dict.keys(), columns=mut_dict.keys())
    cell_feat.to_csv("cell_feat.csv")

    dir_drug = "data/SMILES_GDSC.csv"
    drug_dict = load_drug_smile(dir_drug, col_drug="Drug_CID", col_smi="SMILES_CAN")
    with open('drug_dict', 'wb') as fp:
        pickle.dump(drug_dict, fp)
    
    dir_drug_chembl = "data/SMILES_ChEMBL.csv"
    drug_dict_chembl = load_drug_smile(dir_drug_chembl, col_drug="Molecule_ChEMBL_ID", col_smi="Canonical_SMILES")
    with open('drug_dict_chembl', 'wb') as fp:
        pickle.dump(drug_dict_chembl, fp)
