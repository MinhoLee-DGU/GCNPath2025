...
print("\n### Input File List...")
print("# Cell : {}".format(args.cell))
print("# Drug : {}".format(args.drug))
print("# IC50 : {}".format(args.ic50))
args = create_dir_out(args, dir_out=args.out_dir, suf_param="joblib", retrain=False)


print("\n### Data Processing")
ic50_data = pd.read_csv(args.ic50, header=0, sep="\t")

with open(args.cell, "rb") as f :
    cell_data = pickle.load(f)
    cell_data = {str(k):cell_data[k] for k in cell_data.keys()}

with open(args.drug, "rb") as f :
...
ic50_train, ic50_test = split_ic50(ic50_data, args, choice=args.choice, nth=args.nth, seed=args.seed_split, retrain=True)
train_x, train_y = load_data_classic(ic50_train, cell_data, drug_data, args, no_labels=False)
test_x, test_y = load_data_classic(ic50_test, cell_data, drug_data, args, no_labels=False)
