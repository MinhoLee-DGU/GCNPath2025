...
def main(
    ...
    # PaccMann raises an error when the size of the last batch is 1 [batch_norm]
    # We decided to concat the last and the previous batch [torch.cat]
    bs_last_train = (len(train_dataset.drug_sensitivity_df) % params["batch_size"] != 0)
    bs_last_valid = (len(valid_dataset.drug_sensitivity_df) % params["batch_size"] != 0)
    bs_last_test = (len(test_dataset.drug_sensitivity_df) % params["batch_size"] != 0)
    ...
    if not test_only :
        for epoch in range(params["epoch"]+1, params["epochs"]+1):
            ...
            for ind, (smiles, gep, y) in enumerate(train_loader):
                # if bs_last_train and ind+1==len(train_loader)-1 :
                #     smiles_, gep_, = smiles, gep
                # if bs_last_train and ind+1==len(train_loader) :
                #     smiles = torch.cat([smiles, smiles_])
                #     gep = torch.cat([gep, gep_])
              
                y_hat, pred_dict = model(
                    # torch.squeeze(smiles.to(device)), gep.to(device)
                    smiles.to(device), gep.to(device)
                )
            ...
            with torch.no_grad():
                ...
                for ind, (smiles, gep, y) in enumerate(valid_loader):
                    # if bs_last_valid and ind+1==len(valid_loader)-1 :
                    #     smiles_, gep_ = smiles, gep
                    # if bs_last_valid and ind+1==len(valid_loader) :
                    #     smiles = torch.cat([smiles, smiles_])
                    #     gep = torch.cat([gep, gep_])
    ...
    with torch.no_grad():
        ...
        labels = []
        for ind, (smiles, gep, y) in enumerate(test_loader):
            # if bs_last_test and ind+1==len(test_loader)-1 :
            #     smiles_, gep_ = smiles, gep
            # if bs_last_test and ind+1==len(test_loader) :
            #     smiles = torch.cat([smiles, smiles_])
            #     gep = torch.cat([gep, gep_])
...
if __name__ == "__main__":
    ...
    args.gep_filepath = "data/gene_expression/gdsc-rnaseq_gene-expression.csv"
    args.smi_filepath = "_data/SMILES_GDSC.smi"
    args.gene_filepath = "data/2128_genes.pkl"
    ...
    args.num_workers = 5 if args.device!="cpu" else 0
    
    mode_ = 2
    if mode_==0 :
        dir_out = "Results/Ex_Train"
        dir_ic50 = "_data/Temp_Train"
        idx = [321, 321+320, 321+320+320]
    elif mode_==1 :
        dir_out = "Results/Ex_Valid"
        dir_ic50 = "_data/Temp_Valid"
        idx = [320, 320+321, 320+321+320]
    else :
        dir_out = "Results/Ex_Test"
        dir_ic50 = "_data/Temp_Test"
        idx = [320, 320+320, 320+320+321]
    
    args.model_filepath = ""
    args = create_dir_out_(args, suf_param="pt", dir_out=dir_out)
    ...
    ic50_train, ic50_valid, ic50_test = ic50_data[:idx[0]], ic50_data[idx[0]:idx[1]], ic50_data[idx[1]:idx[2]]
    args = ic50_split_path(args, ic50_train, ic50_valid, ic50_test, dir_ic50=dir_ic50)
