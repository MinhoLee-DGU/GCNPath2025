...
def parse_parameter() :
    ...
    parser.add_argument("-cell", type=str, default="cell_mut_matrix.npy")
    parser.add_argument("-drug", type=str, default="drug_onehot_smiles.npy")
    ...
    parser.add_argument("-dir_param", type=str, default="Results/IC50_GDSC/Normal/param_retrain_0.ckpt")
    
    return parser.parse_args()


args = parse_parameter()
ic50_data = pd.read_csv(args.ic50, header=0, sep="\t")
ic50_data["Norm_IC50"] = norm_ic50(ic50_data[args.col_ic50])
args.col_ic50 = "Norm_IC50"


# Data
cell_data = np.load(args.cell, encoding="latin1", allow_pickle=True).item()
drug_data = np.load(args.drug, encoding="latin1", allow_pickle=True).item()
...
batch_size = 100
test = Batch(ic50_data, cell_data, drug_data, args, batch_size=batch_size, shuffle=False)
...
# # When calling checkpoint like below, .ckpt is not adopted as an input...
# ckpt_state = tf.train.latest_checkpoint(args.dir_param)      # None
# ckpt_state = tf.train.get_checkpoint_state(args.dir_param)   # None

# # Initialize computation graph
# tf.reset_default_graph()

# Generate the trained network
dir_meta = "{}.meta".format(args.dir_param)
saver = tf.train.import_meta_graph(dir_meta)

with tf.Session(config=config) as sess:
    # Load the trained parameters
    saver.restore(sess, args.dir_param)
    ...
    # pred_to_csv_norm(pred_test, ic50_data, args.dir_test)
