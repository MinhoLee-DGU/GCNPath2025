...
class MyDataset_SA(Dataset) :
    ...
    def __init__(self, drug_name2idx, cell_name2idx, ic50_data, args) :
        ...
        self.drug_dict, self.cell_dict = drug_name2idx, cell_name2idx
        self.drug_name = ic50_data[args.col_drug]
        self.cell_name = ic50_data[args.col_cell]
        self.ln_ic50 = ic50_data[args.col_ic50]

    def __len__(self):
        return len(self.ln_ic50)

    def __getitem__(self, index):
        # return (drug_name2idx_dict[self.drug_name[index]], cell_id2idx_dict[self.Cell_line_name[index]], self.value[index])
        return (self.drug_dict[self.drug_name[index]], self.cell_dict[self.cell_name[index]], self.ln_ic50[index])
...
def load_graph_data_SA(args, drug_data, cell_data, edge_index, cluster_predefine, dir_sim) :
  
    drug_graph = list(drug_data.values())
    cell_graph = list(cell_data.values())
    cell_edge_index = torch.tensor(edge_index, dtype=torch.long)
    for u in cell_graph : u.edge_index = cell_edge_index
    
    args.num_feature = 3
    set_random_seed(args.seed)
    model = TGDRP(cluster_predefine, args).to(args.device)
    model.load_state_dict(torch.load(args.weight_tgdrp, map_location=args.device)['model_state_dict'])
    
    parameter = {'drug_emb':model.drug_emb, 'cell_emb':model.cell_emb, 'regression':model.regression}
    drug_graph = Batch.from_data_list(drug_graph).cpu()
    # drug_graph = Batch.from_data_list(drug_graph).to(args.device)
    cell_graph = Batch.from_data_list(cell_graph).to(args.device)
    
    model.GNN_drug = model.GNN_drug.cpu()
    drug_nodes = model.GNN_drug(drug_graph).detach()
    cell_nodes = model.GNN_cell(cell_graph).detach()
    
    print("Drug : {}".format(drug_nodes.shape))
    print("Cell : {}".format(cell_nodes.shape))

    with open(dir_sim, 'rb') as f:
        drug_edges, cell_edges = pickle.load(f)
    drug_edges = torch.tensor(drug_edges, dtype=torch.long).t()
    cell_edges = torch.tensor(cell_edges, dtype=torch.long).t()
