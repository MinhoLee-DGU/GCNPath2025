...
def save_cell_graph(genes_path, save_path):
    ...
    exp = pd.read_csv(os.path.join(genes_path, 'EXP_ComBat.csv'), index_col=0)
    mu = pd.read_csv(os.path.join(genes_path, 'MUT.csv'), index_col=0)
    
    cncat = "tcga" in genes_path
    if not cncat:
        # CNV in log2(CN-Ratio + 1) or 
    ...
    file_scaler_exp = '_data/scaler_exp.pickle'
    
    if not cncat:
        file_scaler_cnv = '_data/scaler_cn.pickle'
    else:
        file_scaler_cnv = '_data/scaler_cncat.pickle'
    
    import pickle
    with open(file_scaler_exp, "rb") as f:
        scaler_exp = pickle.load(f)
    with open(file_scaler_cnv, "rb") as f:
        scaler_cnv = pickle.load(f)
    
    exp = scaler_exp.transform(exp)
    cn = scaler_cnv.transform(cn)
    ...
    if not cncat:
        np.save(os.path.join(save_path, 'cell_feature_all_combat.npy'), cell_dict)
    else:
        np.save(os.path.join(save_path, 'cell_feature_all_cncat_combat.npy'), cell_dict)
