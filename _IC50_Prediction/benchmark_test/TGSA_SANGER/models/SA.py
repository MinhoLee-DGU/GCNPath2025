...
class SA(nn.Module):
    def __init__(self, drug_nodes, cell_nodes, drug_edges, cell_edges, args):
        ...
        self.drug_nodes_feature = drug_nodes.cpu()
    ...
    def forward(self, drug, cell):
        # GPU memory problem in ChEMBL prediction...
        # Avoid unnecessary load of tremendous drug data in GPU
        # Because TGSA in github mode does not utilize drug similarity graph...
        
        drug_id = drug.long()
        cell_id = cell.long()
        drug_x = self.drug_nodes_feature[drug_id].to(self.device)
        cell_x = self.cell_nodes_feature
        drug_x = self.drug_emb(drug_x)
        # drug_x = self.dropout(F.relu(self.drug_conv(drug_x, self.drug_edges)))
        cell_x = self.dropout(F.relu(self.cell_conv_1(cell_x, self.cell_edges)))
        ...
        drug_x = drug_x.squeeze()
        cell_x = cell_x.squeeze()
        x = torch.cat([drug_x, cell_x[cell_id]], -1)
        x = self.regression(x)
        return x
