...
def main(
    ...
    test_rmse_a = np.sqrt(np.mean((predictions - labels) ** 2))
    test_loss_a = test_loss / len(train_loader)
    # test_loss_a = test_loss / len(test_loader)
    logger.info(
        f"\t **** TESTING ****"
        f"loss: {test_loss_a:.5f}, "
    ...
    pred_to_csv(predictions, ic50_data, args.dir_pred)
    logger.info("Done with training, models saved, shutting down.")
...
if __name__ == "__main__":
    ...
    args.dir_param = "{}/param_retrain_seed{}.pt.tar".format(args.dir_out, args.seed)
    args.dir_hparam = "{}/hyper_param_retrain_seed{}.json".format(args.dir_out, args.seed)
    ...
    args = ic50_split_path(args, ic50_data, None, None)
