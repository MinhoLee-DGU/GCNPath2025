...
def main(
    test_sensitivity_filepath, gep_filepath,
    smi_filepath, gene_filepath, smiles_language_filepath, model_filepath, 
    predictions_filepath, params_filepath, args
):
    ...
    # PaccMann_SG showed extremely high RMSEs in ChEMBL external test...
    # However, we found no abnormalities in processing gene expression datasets
    # 
    # breakpoint()
    # exp = pd.read_csv("_data/EXP_CCLE.csv", index_col=0)
    # gene_list = exp.columns.values
    # gene_list_total = test_dataset.gene_expression_dataset.gene_list
    # 
    # gene_diff = list(set(gene_list_total).difference(set(gene_list)))
    # exp[gene_diff] = 0
    # exp = exp[gene_list_total]
    # 
    # cell_list_total = [i for i in test_dataset.gene_expression_dataset.keys()]
    # cell_list_total == exp.index.tolist()   # T
    # np.array_equal(exp.values, test_dataset.gene_expression_dataset.datasets[0][:])   # T
    # 
    # test_dataset.gene_expression_dataset.processing   # {}
    # test_dataset.gene_expression_dataset.min_max   # F
    # test_dataset.gene_expression_dataset.standardize   # F
    # test_dataset.gene_expression_dataset.transform_fn   # function transform_not
