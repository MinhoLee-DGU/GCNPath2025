...
def save_cell_graph(gene_path, save_path, type):
    ref_combat = True
    # file_in =  os.path.join(gene_path, 'EXP.csv')
    # file_in = pd.read_csv(os.path.join(gene_path, 'EXP_GDSC.csv')
    # file_in = pd.read_csv(os.path.join(gene_path, 'CCLE_2369_EXP.csv')
    
    # file_in = os.path.join(gene_path, 'EXP_TCGA_ComBat.csv')
    # file_out = os.path.join(save_path, 'cell_feature_std_{}_tcga_combat.npy').format(type)
    # file_scaler = os.path.join(save_path, 'scaler.pickle')
    
    file_in = os.path.join(gene_path, 'EXP_Liu_2024_ComBat.csv')
    file_out = os.path.join(save_path, 'cell_feature_std_{}_liu24_combat.npy').format(type)
    file_scaler = os.path.join(save_path, 'scaler.pickle')
    ...
    if os.path.exists(file_out):
        ...
        exp = pd.read_csv(file_in, index_col=0)
        index = exp.index
        columns = exp.columns
