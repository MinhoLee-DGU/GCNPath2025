...
boxplot_subtype = function(Pred, SubType_List, col_tn=NULL, name_else="The Others", main=NULL, 
                           width=20, height=18, size_psig=6, exclude_else=T, save=F, ...) {
  
  suppressMessages(library(ggpubr))
  if (is.data.frame(SubType_List)) {
    Subtype = SubType_List[, c("Sample", "Subtype")]
  } else {
    Subtype = stack(SubType_List) %>% setNames(c("Sample", "Subtype"))
  }
  
  Pred = full_join(Pred, Subtype, by="Sample", relationship="many-to-many")
  ...
  Pred = Pred %>% relocate(Subtype, .after=Sample) %>% as.data.frame
  Pred$Subtype = Pred$Subtype %>% factor(levels=c("NMF1", "NMF2", "NMF3", "NMF4"))
  ...
  pl = pl + geom_pwc(data=Pred %>% filter(!!sym(col_tn)=="Tumor"), 
                     label="p.adj.signif", p.adjust.method="fdr", label.size=size_psig,
