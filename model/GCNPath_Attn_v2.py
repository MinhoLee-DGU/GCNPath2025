...
class GCNPath_Attn_v2(Module) :
    ...
    def __init__(self, gnn_cell="GCN", gnn_drug="GCN", mode_cell=None, mode_drug=None,
        ...
        dim_embed_drug = [self.dim_drug_fin + dim_attn, self.dim_embed_drug]
    ...
    def forward(self, cell_data, drug_data, return_attn=False) :
        ...
        # Flatten All Vectors + Drug GNN
        cell = concat[:, :self.n_path, :]
        # [batch, n_path, dim_attn]
        drug_attn = concat[:, self.n_path:, :].squeeze(1)
        # [batch, 1, dim_attn] > [batch, dim_attn] 
        drug = torch.cat([drug, drug_attn], axis=1)
        # [batch, dim_drug_fin+dim_attn]
