...
from torch.nn import Linear, Dropout, BatchNorm1d, Parameter, Sequential, Module, ModuleList
...
class DrugLayer(Module) :
    ...
    def __init__(self, gnn="GAT", mode=None, dim=[128, 128, 128, 128], 
                 ...
                 act=ReLU(), norm=True, drop=0.2, norm_graph=False, 
                 dmpnn=False, iter_dmpnn=10, **kwargs) :
        ...
        if self.attn_mode in [3, 4] :
            ...
            self.drug_layer = SSIModule(n_layer=n_layer, dim_in=dim[0], dim_out=dim[1], 
                                        ...
                                        act=act, norm=norm, drop=drop, norm_graph=norm_graph, 
                                        use_gin=use_gin, dmpnn=dmpnn, iter_dmpnn=iter_dmpnn, **kwargs)
        else :
            if gnn!="linear" :
                self.drug_layer = GNNLayer(mode=mode, gnn=gnn, dim=dim, act=act, norm=norm, drop=drop, 
                                           norm_graph=norm_graph, heads=h_gat, concat=concat_gat, edge_dim=n_drug_edge, **kwargs)
            else :
...
class SSIModule(Module) :
    ...
    def __init__(self, n_layer=3, dim_in=128, dim_out=128, heads=4, concat=True, edge_dim=16, 
                 act=ELU(1.0), norm=True, drop=0.2, norm_graph=False, use_gin=0, dmpnn=False, iter_dmpnn=10) :
        ...
        if self.dmpnn :
            dim_pre = [dim_in, dim_out, dim_out, dim_out]
            self.pre_dmpnn = MLP(dim=dim_pre, act=act, norm=norm, drop=drop)
            dim_in = dim_out
        
        for _ in range(n_layer) :
            layer = SSILayer(dim_in, dim_out, heads=heads, concat=concat, edge_dim=edge_dim,  
                             act=act, norm=norm, drop=drop, norm_graph=norm_graph, 
                             use_gin=use_gin, dmpnn=dmpnn, iter_dmpnn=iter_dmpnn)
...
class SSILayer(Module) :
    ...
    def __init__(self, dim_in=128, dim_out=128, heads=4, concat=True, act=ELU(1.0), 
                 norm=True, drop=0.2, norm_graph=False, edge_dim=10, use_gin=0, dmpnn=False, iter_dmpnn=10) :
        ...
        if use_gin==0 :
            ...
            conv = GATv2Conv(dim_in, dim_mid, heads=heads, concat=concat, edge_dim=edge_dim)
            ...
            conv = GINConv(gin_linear)
        else :
            # [use_gin>1] GINEConv
            gin_linear = Sequential(Linear(dim_in, dim_out), ReLU(), Linear(dim_out, dim_out))
            conv = GINEConv(gin_linear, edge_dim=edge_dim)
        if self.dmpnn :
            conv = DMPNN(dim_in, dim_out, edge_dim=edge_dim, 
                         n_iter=iter_dmpnn, act=act, drop=drop, norm=norm)
        ...
        # [GNN] Conv - Norm - Act - Drop
        # Do not apply dropout in GNN...
        
        norm_layer_def = norm_layer if not norm_graph else norm_layer_graph
        norm = norm_layer_def(norm, dim=dim_out)
        self.conv = DeepGCNLayerDef(conv=conv, norm=norm, act=act, dropout=False, block="plain")
...
class DMPNN(Module) :
    ...
    def __init__(self, dim_in=128, dim_out=128, edge_dim=10, 
                 n_iter=10, act=ELU(1.0), drop=0.2, norm=True) :
        
        super().__init__()
        self.n_iter = n_iter
        self.act_gout = Tanh()
        
        self.lin_u = MLP(dim=[dim_in, dim_out], act=act, drop=drop, norm=norm)
        self.lin_v = MLP(dim=[dim_in, dim_out], act=act, drop=drop, norm=norm)
        self.lin_edge = MLP(dim=[edge_dim, dim_out], act=act, drop=drop, norm=norm)
        ...
        glorot(self.a)
        self.lin_block = MLP(dim=[dim_out, dim_out], act=act, norm=norm, drop=drop)
    ...
    def forward(self, x, edge_index, edge_attr, edge_index_bond, edge_index_batch) :
        ...
        for n in range(self.n_iter):
            out = scatter(out[edge_index_bond[0]], edge_index_bond[1], 
