...
class FeedForwardVar(Module) :

    def __init__ (self, dim_in=128, dim_ffnn=256, dim_out=None, drop=0.1, act_gelu=False) :
        super().__init__()
        self.dropout = Dropout(p=drop)
        if dim_out is None : dim_out = dim_in
        self.fc1 = Linear(dim_in, dim_ffnn)
        self.fc2 = Linear(dim_ffnn, dim_out)
        self.act = ReLU() if not act_gelu else GELU()
...
class EncoderLayerVar(Module) :

    def __init__(self, n_heads=4, dim_cell=48, dim_drug=128, 
                 dim_attn=64, coef_ffnn=4, drop=0.1, act_gelu=False, n_path=292) :
        
        super().__init__()
        self.n_path = n_path
        self.dropout = Dropout(p=drop)
        self.proj_cell = Linear(dim_cell, dim_attn)
        self.proj_drug = Linear(dim_drug, dim_attn)
        
        self.layerNorm = LayerNorm(dim_attn)
        self.layerNorm_cell = LayerNorm(dim_cell)
        self.layerNorm_drug = LayerNorm(dim_drug)
        
        dim_ffnn_cell = coef_ffnn * dim_attn
        # 48 > 64 > 4*48 [192] > 48 [dim_cell, X]
        # 48 > 64 > 4*64 [256] > 48 [dim_attn, O]
        dim_ffnn_drug = coef_ffnn * dim_attn
        # 128 > 64 > 4*128 [512] > 128 [dim_drug, X]
        # 128 > 64 > 4*64 [256] > 128 [dim_attn, O]
        
        self.ffnn_cell = FeedForwardVar(dim_attn, dim_ffnn_cell, dim_out=dim_cell, drop=drop, act_gelu=act_gelu)
        self.ffnn_drug = FeedForwardVar(dim_attn, dim_ffnn_drug, dim_out=dim_drug, drop=drop, act_gelu=act_gelu)
        self.self_attn = MultiheadAttention(embed_dim=dim_attn, num_heads=n_heads, dropout=drop, batch_first=True)

    def forward(self, cell, drug) :
        cell_attn = self.proj_cell(cell)
        # [batch, n_path, dim_attn]
        drug_attn = self.proj_drug(drug)
        # [batch, n_subs, dim_attn]
        out = torch.cat([cell_attn, drug_attn], axis=1)
        # [batch, n_path+n_subs, dim_attn]
        out, attn = self.self_attn(query=out, key=out, value=out, attn_mask=None)
        # [batch, n_path+n_subs, dim_attn]
        
        out = self.layerNorm(out + self.dropout(out))
        # [batch, n_path+n_subs, dim_attn]
        cell_ = self.ffnn_cell(out[:, :self.n_path, :])
        # [batch, n_path, dim_cell]
        drug_ = self.ffnn_drug(out[:, self.n_path, :])
        # [batch, n_path, dim_drug]
        cell = self.layerNorm_cell(cell + self.dropout(cell_))
        # [batch, n_path, dim_cell]
        drug = self.layerNorm_drug(drug + self.dropout(drug_))
        # [batch, n_path, dim_drug]
        
        return cell, drug, attn
...
class EncoderVar(Module) :
    ...
    def forward(self, cell, drug) :
        for layer in self.layers :
            cell, drug, attn = layer(cell, drug)
        return cell, drug, attn
